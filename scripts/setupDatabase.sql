-- =============================================
-- VTON Backend Database Setup Script
...
-- Run these commands in Supabase Dashboard Storage section or use the API:

-- 1. Create bucket named 'vton-assets'
-- 2. Set up access policies:
--    - Public read access for garments/
--    - Private access for user-uploads/ and try-on-results/

-- Example bucket structure:
-- vton-assets/
-- ├── garments/           (public - clothing items)
-- ├── user-uploads/       (private - user original photos)
-- │   └── {user_id}/
-- │       └── {timestamp}_original.jpg
-- └── try-on-results/     (private - generated results)
--     └── {user_id}/
...
-- Verify setup
SELECT 'VTON Database Setup Complete!' as status;
